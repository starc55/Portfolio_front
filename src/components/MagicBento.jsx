...
const cardData = [
...

  {
    color: "#060010",
    description:
      "💻 I’m a Frontend Developer specializing in modern and creative web experiences. I work with React, JavaScript, CSS and design tools like Figma and Canva to build visually appealing and user-friendly websites.",
    label: "Know about me",
  },
...
const ParticleCard = ({
  ...
  useEffect(() => {
    ...
    if (isMobile) {
      const bars = element.querySelectorAll(".skill-progress-fill");
      bars.forEach((bar) => {
...
const GlobalSpotlight = ({
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e) => {
      ...
      const cards = gridRef.current.querySelectorAll(".card_magic");
        ...
        {cardData.map((card, index) => {
          ...
          if (enableStars) {
            return (
              ...
              >
                {card.video ? (
                  // agar 3-karta bo'lsa → video
                  <VideoCard src={card.video} poster={card.poster} />
                ) : (
                  // qolgan kartalar → matn
                  <>
                    <div className="card__header">
                      <div className="card__label">{card.label}</div>
                    </div>
